<div class="min-h-screen bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50">
    ...
    <div class="bg-gradient-to-r from-purple-500/10 via-indigo-500/10 to-blue-500/10 backdrop-blur-sm z-40 border-b border-purple-200">
        <div class="max-w-7xl mx-auto px-4 py-8">
            <div class="text-center mb-8">
                <div class="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r from-purple-500 to-indigo-600 rounded-full mb-6 shadow-2xl">
                    <span class="text-4xl">🌸</span>
                </div>
                <h1 class="text-4xl lg:text-5xl font-display font-bold bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 bg-clip-text text-transparent mb-4">
                    Collection Parfums
                </h1>
            ...
            <div class="max-w-2xl mx-auto">
                <div class="relative">
                    ...
                    @if($search)
                        <button wire:click="$set('search', '')" class="absolute right-5 top-1/2 transform -translate-y-1/2 text-purple-400 hover:text-purple-600">
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-80 {{ $showFilters ? 'block' : 'hidden lg:block' }}">
                <div class="bg-white/80 backdrop-blur-sm rounded-3xl p-6 shadow-2xl border border-purple-100 sticky top-32">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                @foreach($volumes as $volume)
                                    <button wire:click="$set('selectedVolume', '{{ $selectedVolume === $volume ? '' : $volume }}')"
                    ...
                    <button wire:click="$set('search', ''); $set('selectedBrand', ''); $set('selectedConcentration', ''); $set('selectedVolume', ''); $set('minPrice', 0); $set('maxPrice', 1000)" 
                ...
                <div class="flex items-center justify-between mb-8">
                    <div>
                        <p class="text-gray-600">
                            <span class="font-bold text-purple-600 text-lg">{{ $products->total() }}</span> parfums d'exception
                        </p>
                        @if($search)
                    ...
                    <div class="{{ $viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8' : 'space-y-6' }}">
                        @foreach($products as $product)
                            <div class="group" 
                                ...
                                @if($viewMode === 'grid')
                                    {{-- Vue grille parfums --}}
                                    <div class="bg-white/80 backdrop-blur-sm rounded-3xl overflow-hidden shadow-xl border border-purple-100 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-3 hover:scale-105">
                                        {{-- Image parfum --}}
                                        <div class="relative aspect-square overflow-hidden cursor-pointer" 
                                            ...
                                            <div class="absolute top-4 left-4 flex flex-wrap gap-2">
                                                ...
                                                @if($product->concentration)
                                                    <span class="bg-gradient-to-r from-purple-500 to-indigo-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg">
                                            ...
                                            {{-- Actions rapides --}}
                                            <div class="absolute top-4 right-4 flex flex-col space-y-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0">
                                                {{-- Favoris --}}
                                        ...
                                        <div class="p-6">
                                            {{-- Marque --}}
                                            <p class="text-sm font-semibold text-purple-600 mb-2 uppercase tracking-wide">
                                                {{ $product->brand->name }}
                                            </p>
                                            ...
                                            <div class="flex items-center space-x-3 mb-3 text-sm text-gray-600">
                                                @if($product->volume)
                                                    <span class="bg-purple-50 text-purple-700 px-2 py-1 rounded-lg font-medium">
                                                        🧴 {{ $product->volume }}
                                                    </span>
                                                @endif
                                                @if($product->concentration)
                                                    <span class="bg-indigo-50 text-indigo-700 px-2 py-1 rounded-lg font-medium">
                                                        💎 {{ $product->concentration }}
                                                    </span>
                                                @endif
                                            </div>
                                            ...
                                            @if($product->is_in_stock)
                                                <button wire:click="addToCart({{ $product->id }})"
                                                    ...
                                                    <span x-show="!loading" class="flex items-center justify-center space-x-2">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-1.1 5M7 13h10m0 0l1.1 5M17 17a2 2 0 11-4 0 2 2 0 014 0zM9 17a2 2 0 11-4 0 2 2 0 014 0z"></path>
                                                        </svg>
                                            ...
                                            @else
                                                <button disabled class="w-full bg-gray-300 text-gray-500 py-3 px-4 rounded-2xl font-semibold cursor-not-allowed">
                                                    Rupture de stock
                                                </button>
                                            @endif
                                        </div>
                                ...
                                @else
                                    {{-- Vue liste parfums --}}
                                    <div class="bg-white/80 backdrop-blur-sm rounded-3xl overflow-hidden shadow-xl border border-purple-100 hover:shadow-2xl transition-all duration-300">
                                        <div class="flex flex-col md:flex-row">
                                            ...
                                            <div class="flex-1 p-6">
                                                <div class="flex flex-col md:flex-row md:items-center justify-between h-full">
                                                    <div class="flex-1">
                                                        {{-- Marque --}}
                                                        <p class="text-sm font-semibold text-purple-600 mb-2 uppercase tracking-wide">
                                                            {{ $product->brand->name }}
                                                        ...
                                                        <div class="flex flex-wrap items-center gap-2 mb-3">
                                                            @if($product->volume)
                                                                <span class="bg-purple-50 text-purple-700 px-2 py-1 rounded-lg text-xs font-medium">
                                                                    🧴 {{ $product->volume }}
                                                                </span>
                                                            @endif
                                                            @if($product->concentration)
                                                                <span class="bg-indigo-50 text-indigo-700 px-2 py-1 rounded-lg text-xs font-medium">
                                                                    💎 {{ $product->concentration }}
                                                                </span>
                                                            @endif
                                                    ...
                                                    <div class="md:text-right md:ml-6">
                                                        {{-- Prix --}}
                                                        <div class="mb-4">
                                                            <div class="flex md:flex-col items-center md:items-end space-x-2 md:space-x-0">
                                                                <span class="text-2xl font-bold text-gray-900">{{ $product->price }}€</span>
                                                                @if($product->original_price && $product->price < $product->original_price)
                                                                    <div class="md:mt-1">
                                                                        <span class="text-lg text-gray-500 line-through">{{ $product->original_price }}€</span>
                                                                        <span class="text-sm font-semibold text-red-600 bg-red-100 px-2 py-0.5 rounded-full ml-2">
                                                                            -{{ $product->discount_percentage }}%
                                                                        </span>
                                                                    </div>
                                                                @endif
                                                        ...
                                                        <div class="flex md:flex-col space-x-2 md:space-x-0 md:space-y-2">
                                                            ...
                                                            @else
                                                                <button disabled class="flex-1 md:w-auto bg-gray-300 text-gray-500 py-2 px-4 rounded-xl font-semibold cursor-not-allowed">
                                                                    Rupture
                                                                </button>
                                                            @endif
                                                            ...
                                                            <div class="flex space-x-2">
                                                                ...
                                                                <button wire:click="toggleWishlist({{ $product->id }})"
                                                                        :class="inWishlist ? 'bg-red-500 text-white' : 'bg-purple-100 text-purple-600'"
                                                                ...
                                                                {{-- Contact --}}
                                                                <button wire:click="contactWhatsApp({{ $product->id }})"
    ...
    @if($showQuickView && $selectedProduct)
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" 
             x-data="{ show: @entangle('showQuickView') }"
             x-show="show"
            ...
            <div class="bg-white rounded-3xl max-w-5xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-purple-100"
                ...
                {{-- Header modal --}}
                <div class="sticky top-0 bg-gradient-to-r from-purple-500/10 to-indigo-500/10 backdrop-blur-sm border-b border-purple-200 p-6 rounded-t-3xl z-10">
                ...
                <div class="p-8">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        ...
                        <div class="space-y-6">
                            {{-- Image principale --}}
                            <div class="aspect-square rounded-3xl overflow-hidden bg-gradient-to-br from-purple-50 to-indigo-50 shadow-xl">
                            ...
                            @if($selectedProduct->images->count() > 1)
                                <div class="grid grid-cols-4 gap-3">
                                    @foreach($selectedProduct->images->take(4) as $image)
                                        <div class="aspect-square rounded-xl overflow-hidden bg-purple-50 cursor-pointer hover:opacity-75 transition-opacity shadow-lg">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <div class="flex items-center space-x-3 mb-6">
                                    <span class="text-4xl font-bold text-gray-900">{{ $selectedProduct->price }}€</span>
                                    @if($selectedProduct->original_price && $selectedProduct->price < $selectedProduct->original_price)
                                        <span class="text-2xl text-gray-500 line-through">{{ $selectedProduct->original_price }}€</span>
                            ...
                            @if($selectedProduct->short_description)
                                <div class="bg-purple-50 rounded-2xl p-6">
                                    <h3 class="font-semibold text-purple-900 mb-3 flex items-center">
                                        <span class="w-6 h-6 bg-purple-200 rounded-full flex items-center justify-center mr-2">
                                            <span class="text-xs">📝</span>
                                        </span>
                                        Description
                                    </h3>
                            ...
                            <div class="flex items-center justify-between py-4 border-t border-purple-200">
                                ...
                                @if($selectedProduct->rating_average > 0)
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <span class="text-sm text-gray-500">({{ $selectedProduct->reviews_count }} avis)</span>
                                    </div>
                                @endif
                            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        sliders.forEach(slider => {
            slider.style.background = `linear-gradient(to right, #8B5CF6 0%, #8B5CF6 ${(slider.value - slider.min) / (slider.max - slider.min) * 100}%, #e2e8f0 ${(slider.value - slider.min) / (slider.max - slider.min) * 100}%, #e2e8f0 100%)`;
            
            slider.addEventListener('input', function() {
                this.style.background = `linear-gradient(to right, #8B5CF6 0%, #8B5CF6 ${(this.value - this.min) / (this.max - this.min) * 100}%, #e2e8f0 ${(this.value - this.min) / (this.max - this.min) * 100}%, #e2e8f0 100%)`;
            });
        });
    });
</script>
