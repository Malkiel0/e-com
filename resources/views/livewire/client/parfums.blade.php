<div class="min-h-screen bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50">
    ...
    {{-- Message de succès panier SIMPLIFIÉ (comme Dashboar) --}}
    @if ($showCartSuccess)
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <button wire:click="sortBy('name')"
                                class="p-3 rounded-xl text-sm font-medium transition-all duration-200 {{ $sortBy === 'name' ? 'bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-lg' : 'bg-purple-50 text-purple-700 hover:bg-purple-100' }}">
                            ...
                            <button wire:click="sortBy('price')"
                                class="p-3 rounded-xl text-sm font-medium transition-all duration-200 {{ $sortBy === 'price' ? 'bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-lg' : 'bg-purple-50 text-purple-700 hover:bg-purple-100' }}">
                                <span class="flex items-center justify-center space-x-1">
                                    <span>Prix</span>
                                    @if($sortBy === 'price')
                                        <span class="text-xs">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                            ...
                            <button wire:click="sortBy('rating')"
                                class="p-3 rounded-xl text-sm font-medium transition-all duration-200 {{ $sortBy === 'rating' ? 'bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-lg' : 'bg-purple-50 text-purple-700 hover:bg-purple-100' }}">
                                <span class="flex items-center justify-center space-x-1">
                                    <span>Note</span>
                                    @if($sortBy === 'rating')
                                        <span class="text-xs">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                            ...
                            <button wire:click="sortBy('sales')"
                                class="p-3 rounded-xl text-sm font-medium transition-all duration-200 {{ $sortBy === 'sales' ? 'bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-lg' : 'bg-purple-50 text-purple-700 hover:bg-purple-100' }}">
                                <span class="flex items-center justify-center space-x-1">
                                    <span>Ventes</span>
                                    @if($sortBy === 'sales')
                                        <span class="text-xs">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                    ...
                    <div
                        ...
                        @foreach ($products as $product)
                            <div class="group" data-product-id="{{ $product->id }}" x-data="{
                                ...
                                @else
                                    {{-- Vue liste parfums (identique à l'original) --}}
                                    <div
    ...
    {{-- Modal Quick View CORRIGÉE avec scroll automatique --}}
    @if ($showQuickView && $selectedProduct)
        <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] overflow-y-auto"
            ...
            <div class="min-h-screen flex items-start justify-center p-4 pt-16">
                <div class="bg-white rounded-3xl w-full max-w-6xl mx-auto shadow-2xl border border-purple-100 max-h-[85vh] overflow-y-auto"
                    x-transition:enter="transition ease-out duration-300" 
                ...
                {{-- Header modal FIXE --}}
                <div class="sticky top-0 bg-gradient-to-r from-purple-500/10 to-indigo-500/10 backdrop-blur-sm border-b border-purple-200 p-4 md:p-6 rounded-t-3xl z-10">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            <div class="w-8 md:w-10 h-8 md:h-10 bg-gradient-to-r from-purple-500 to-indigo-600 rounded-full flex items-center justify-center">
                                <span class="text-white text-sm md:text-base">🌸</span>
                            </div>
                            <h2 class="text-lg md:text-2xl font-bold bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent">
                                Aperçu Parfum
                            </h2>
                        </div>
                        <button wire:click="closeQuickView"
                            class="p-2 hover:bg-purple-100 rounded-full transition-colors text-purple-600 z-20">
                ...
                <div class="p-4 md:p-8">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10">
                        ...
                        <div class="space-y-4 md:space-y-6">
                            {{-- Image principale --}}
                            <div class="aspect-square rounded-2xl md:rounded-3xl overflow-hidden bg-gradient-to-br from-purple-50 to-indigo-50 shadow-xl relative cursor-zoom-in"
                                @click="toggleZoom()">
                                ...
                                @else
                                    <div class="w-full h-full bg-gradient-to-br from-purple-100 via-indigo-100 to-blue-100 flex items-center justify-center">
                            ...
                            @if ($selectedProduct->images->count() > 1)
                                <div class="grid grid-cols-4 gap-2 md:gap-3">
                                    @foreach ($selectedProduct->images->take(4) as $image)
                                        <div class="aspect-square rounded-lg md:rounded-xl overflow-hidden bg-purple-50 cursor-pointer hover:opacity-75 transition-all shadow-lg border-2"
                        ...
                        <div class="space-y-4 md:space-y-6">
                            ...
                            <div>
                                <div class="flex items-center space-x-3 mb-3">
                                    <span class="text-base md:text-lg font-bold text-purple-600 uppercase tracking-wide">
                                        {{ $selectedProduct->brand->name }}
                                    </span>
                                    <span class="text-xs md:text-sm text-gray-500 bg-purple-50 px-2 md:px-3 py-1 rounded-full">
                            ...
                            @if ($selectedProduct->short_description)
                                <div class="bg-purple-50 rounded-2xl p-4 md:p-6">
                                    <h3 class="font-semibold text-purple-900 mb-3 flex items-center text-sm md:text-base">
                                        <span class="w-5 h-5 md:w-6 md:h-6 bg-purple-200 rounded-full flex items-center justify-center mr-2">
                                            <span class="text-xs">📝</span>
                                        </span>
                                        Description
                                    </h3>
                                    <p class="text-purple-800 text-sm md:text-base">{{ $selectedProduct->short_description }}</p>
                                </div>
                            @endif

                            {{-- Caractéristiques parfum RESPONSIVE --}}
                            <div class="grid grid-cols-2 gap-3 md:gap-4">
                                @if ($selectedProduct->volume)
                                    <div class="bg-indigo-50 rounded-xl md:rounded-2xl p-3 md:p-4 text-center">
                                        <div class="text-xl md:text-2xl mb-2">🧴</div>
                                        <span class="text-xs md:text-sm text-indigo-600 font-medium">Volume</span>
                                        <p class="font-bold text-indigo-900 text-sm md:text-base">{{ $selectedProduct->volume }}</p>
                                    </div>
                                @endif

                                @if ($selectedProduct->concentration)
                                    <div class="bg-purple-50 rounded-xl md:rounded-2xl p-3 md:p-4 text-center">
                                        <div class="text-xl md:text-2xl mb-2">💎</div>
                                        <span class="text-xs md:text-sm text-purple-600 font-medium">Concentration</span>
                                        <p class="font-bold text-purple-900 text-sm md:text-base">{{ $selectedProduct->concentration }}</p>
                                    </div>
                                @endif
                            </div>
                            ...
                            @if ($selectedProduct->fragrance_notes)
                                <div class="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-2xl p-4 md:p-6">
                                    ...
                                    <div class="space-y-3">
                                        @foreach ($selectedProduct->fragrance_notes as $noteType => $notes)
                                            <div class="flex items-start space-x-3">
                                                <span class="text-xs md:text-sm font-semibold text-purple-600 min-w-[80px] md:min-w-[100px] capitalize">{{ $noteType }}:</span>
                                                <span class="text-xs md:text-sm text-gray-700 flex-1">{{ is_array($notes) ? implode(', ', $notes) : $notes }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                            ...
                            <div class="space-y-3 md:space-y-4">
                                @if ($selectedProduct->is_in_stock)
                                    <button wire:click="addToCart({{ $selectedProduct->id }})"
                                        class="w-full py-3 md:py-4 px-4 md:px-6 rounded-xl md:rounded-2xl font-bold text-white text-base md:text-lg transition-all transform hover:scale-105 shadow-xl hover:shadow-2xl bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700">
                                        🛒 Ajouter au panier
                                    </button>
                                @else
                                    <button disabled
                                ...
                                <div class="grid grid-cols-2 gap-3 md:gap-4">
                                    <button wire:click="toggleWishlist({{ $selectedProduct->id }})"
                                        class="flex items-center justify-center space-x-2 py-2 md:py-3 px-3 md:px-4 rounded-lg md:rounded-xl border-2 border-purple-300 hover:border-red-500 hover:text-red-500 transition-all text-sm md:text-base">
...
{{-- Scripts SIMPLIFIÉS (comme Dashboar) --}}
@push('scripts')
    <script>
        document.addEventListener('livewire:init', () => {
            console.log('🚀 Livewire Parfums initialisé');

            // ✅ ÉCOUTEUR WHATSAPP (identique à Dashboar)
            ...
            Livewire.on('modal-opened', () => {
                console.log('📱 Modal ouverte - scroll vers le haut');
        ...
        // ✅ SYSTÈME DE NOTIFICATION (identique à Dashboar)
        ...
        function showCartNotification() {
            ...
            const message = notificationElement ? notificationElement.dataset.message : 'Parfum ajouté au panier !';
                ...
                <div class="flex items-start space-x-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between mb-2">
                            <h4 class="font-bold text-white text-base">🌸 Parfum ajouté !</h4>
        ...
        function updateSliderBackground(slider) {
            ...
            slider.style.background = `linear-gradient(to right, #8B5CF6 0%, #8B5CF6 ${percentage}%, #e2e8f0 ${percentage}%, #e2e8f0 100%)`;
        }
...
{{-- Styles AMÉLIORÉS (identiques à Dashboar) --}}
@push('styles')
    <style>
        .line-clamp-2 {
            display: -webkit-box;
        ...
        .purple-slider:focus::-webkit-slider-thumb {
            box-shadow: 0 0 0 3px rgba(139, 92, 246, 0.2);
        }

        /* ✅ NOTIFICATION TOAST (identique à Dashboar) */
