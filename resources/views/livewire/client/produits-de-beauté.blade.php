<div class="min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-orange-50">
    ...
    <div class="bg-gradient-to-r from-pink-500/10 via-rose-500/10 to-orange-500/10 backdrop-blur-sm z-40 border-b border-pink-200">
        <div class="max-w-7xl mx-auto px-4 py-8">
            <div class="text-center mb-8">
                <div class="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r from-pink-500 to-orange-500 rounded-full mb-6 shadow-2xl">
                    <span class="text-4xl">💄</span>
                </div>
                <h1 class="text-4xl lg:text-5xl font-display font-bold bg-gradient-to-r from-pink-600 via-rose-600 to-orange-600 bg-clip-text text-transparent mb-4">
                    Collection Beauté
                </h1>
            ...
            <div class="max-w-2xl mx-auto">
                <div class="relative">
                    ...
                    @if($search)
                        <button wire:click="$set('search', '')" class="absolute right-5 top-1/2 transform -translate-y-1/2 text-pink-400 hover:text-pink-600">
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-80 {{ $showFilters ? 'block' : 'hidden lg:block' }}">
                <div class="bg-white/80 backdrop-blur-sm rounded-3xl p-6 shadow-2xl border border-pink-100 sticky top-32">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-1 gap-2">
                                @foreach($skinTypes as $skinType)
                                    <button wire:click="$set('selectedSkinType', '{{ $selectedSkinType === $skinType ? '' : $skinType }}')"
                    ...
                    <button wire:click="$set('search', ''); $set('selectedCategory', ''); $set('selectedBrand', ''); $set('selectedSkinType', ''); $set('minPrice', 0); $set('maxPrice', 1000)" 
                ...
                <div class="flex items-center justify-between mb-8">
                    <div>
                        <p class="text-gray-600">
                            <span class="font-bold text-pink-600 text-lg">{{ $products->total() }}</span> produits de beauté
                        </p>
                        @if($search)
                    ...
                    <div class="{{ $viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8' : 'space-y-6' }}">
                        @foreach($products as $product)
                            <div class="group" 
                                ...
                                @if($viewMode === 'grid')
                                    {{-- Vue grille beauté --}}
                                    <div class="bg-white/80 backdrop-blur-sm rounded-3xl overflow-hidden shadow-xl border border-pink-100 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-3 hover:scale-105">
                                        ...
                                        <div class="relative aspect-square overflow-hidden cursor-pointer" 
                                            ...
                                            {{-- Overlay gradient --}}
                                            <div class="absolute inset-0 bg-gradient-to-t from-pink-900/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                                        ...
                                        <div class="p-6">
                                            {{-- Marque --}}
                                            <p class="text-sm font-semibold text-pink-600 mb-2 uppercase tracking-wide">
                                                {{ $product->brand->name }}
                                            </p>
                                            ...
                                            <div class="flex items-center space-x-3 mb-3 text-sm text-gray-600">
                                                <span class="bg-pink-50 text-pink-700 px-2 py-1 rounded-lg font-medium">
                                                    {{ $product->category->name }}
                                                </span>
                                                @if($product->skin_type)
                                                    <span class="bg-rose-50 text-rose-700 px-2 py-1 rounded-lg font-medium">
                                                        🌟 {{ $product->skin_type }}
                                                    </span>
                                                @endif
                                            </div>
                                ...
                                @else
                                    ...
                                    <div class="bg-white/80 backdrop-blur-sm rounded-3xl overflow-hidden shadow-xl border border-pink-100 hover:shadow-2xl transition-all duration-300">
                                                ...
                                                <div class="flex flex-col md:flex-row md:items-center justify-between h-full">
                                                    <div class="flex-1">
                                                        ...
                                                        <div class="flex flex-wrap items-center gap-2 mb-3">
                                                            <span class="bg-pink-50 text-pink-700 px-2 py-1 rounded-lg text-xs font-medium">
                                                                {{ $product->category->name }}
                                                            </span>
                                                            @if($product->skin_type)
                                                                <span class="bg-rose-50 text-rose-700 px-2 py-1 rounded-lg text-xs font-medium">
                                                                    🌟 {{ $product->skin_type }}
                                                                </span>
                                                            @endif
                                                        ...
                                                        <div class="flex md:flex-col space-x-2 md:space-x-0 md:space-y-2">
                                                            ...
                                                            <div class="flex space-x-2">
                                                                ...
                                                                <button wire:click="toggleWishlist({{ $product->id }})"
                                                                        :class="inWishlist ? 'bg-red-500 text-white' : 'bg-pink-100 text-pink-600'"
    ...
    @if($showQuickView && $selectedProduct)
        {{-- Modal quick view avec couleurs pink/orange... (code identique aux autres modals mais avec les bonnes couleurs) --}}
    @endif
</div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        sliders.forEach(slider => {
            slider.style.background = `linear-gradient(to right, #EC4899 0%, #EC4899 ${(slider.value - slider.min) / (slider.max - slider.min) * 100}%, #e5e7eb ${(slider.value - slider.min) / (slider.max - slider.min) * 100}%, #e5e7eb 100%)`;
            
            slider.addEventListener('input', function() {
                this.style.background = `linear-gradient(to right, #EC4899 0%, #EC4899 ${(this.value - this.min) / (this.max - this.min) * 100}%, #e5e7eb ${(this.value - this.min) / (this.max - this.min) * 100}%, #e5e7eb 100%)`;
            });
        });
    });
</script>
